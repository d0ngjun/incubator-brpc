...
DEFINE_int32(minloglevel_to_colorlogserver, 2, "Any log at or above this level will be "
             "sent to color log server. Anything below this level will be silently ignored. "
             "0=INFO 1=NOTICE 2=WARNING 3=ERROR 4=FATAL");
...
DEFINE_string(log_directory, "/data/log", "log files directory");

const char* const log_severity_names[LOG_NUM_SEVERITIES] = {
    "KERNEL", "DEBUG", "INFO", "WARNING", "ERROR", "FATAL" };
...
bool LogIsOn(int severity) {
    ...
    return severity >= GetMinLogLevel() || ((tls_header && tls_header->log_coloring()) && (severity >= FLAGS_minloglevel_to_colorlogserver));
}
...
// use this as default log prefix
// define it in logging namespace so that other sink could use
...
bool MakeDir(const char* szDirPath)
{
...
bool InitializeLogFileHandle() {
    ...
    if (!log_file_name) {
        ...
        char buffer[260];
        snprintf(buffer, 260, "%s/log%04d%02d%02d%02d.log.%u", 
                 log_file_dir->c_str(),
                 (now_tm.tm_year+1900), (now_tm.tm_mon+1),
                 now_tm.tm_mday, now_tm.tm_hour, CurrentProcessId());

        log_file_name = new PathString(buffer);
    }
...
#elif defined(OS_POSIX)
        if (!butil::PathExists(butil::FilePath(log_file_dir->c_str()))) {
            if (!MakeDir(log_file_dir->c_str())) {
                fprintf(stderr, "Fail to mkdir %s", log_file_dir->c_str());
                return false;
            }   
        }
...
bool BaseInitLoggingImpl(const LoggingSettings& settings) {
    ...
    if (!settings.module_name.empty() && !settings.logd_module_name.empty()) {
        if (ColorLogClient::GetInstance()->Init(settings.module_name, settings.logd_module_name) == 0) {
    ...
    if (!butil::PathExists(butil::FilePath(FLAGS_log_directory))) {
        butil::File::Error err;
        if (!butil::CreateDirectoryAndGetError(butil::FilePath(FLAGS_log_directory), &err)) {
            std::cerr << "create log directy failed, path = " << FLAGS_log_directory
                << " , error = " << err;
            return false;
        }
    }
    ...
    if (settings.async) {
        SetLogSink(AsyncLogSink::GetInstance());
    }
...
inline LogStream* CreateLogStream(const PathChar* file, int line,
    ...
    if (stream->empty()) {
        stream->SetPosition(file, line, func_name, severity);
    }
    return stream;
}
...
public:
    ...
    bool OnLogMessage(int severity, const char* file, int line,
        ...
        if ((logging_destination & LOG_TO_FILE) != 0) {
            ...
            log_file_->Write(log.data(), log.size(), true);
        }
        return true;
    }
private:
    DefaultLogSink() : log_file_(nullptr) {}
    DefaultLogSink(const DefaultLogSink&) = delete;
    DefaultLogSink& operator=(const DefaultLogSink&) = delete;
    ~DefaultLogSink() { 
        delete log_file_;
        log_file_ = nullptr;
    }
...
void LogStream::FlushWithoutReset() {
    ...
    *this << std::ends;
    ...
    // only handle LOG(xxx)
    // there is no way to map a negative integer to KERNEL(0),DEBUG(1)...
    int level = severity();
    ColorLogHeader* tls_header = ColorLogHeader::bls_color_log_header();
    if (has_module_name && tls_header && tls_header->log_coloring() && level >= FLAGS_minloglevel_to_colorlogserver) { //todo add filter here
        if (FLAGS_lockfree_queue_or_not) {
            ColorLogClient::GetInstance()->SendLogLockFree(severity(), file(), line(), func_name(), std::move(content_iobuf()));
        } else {
            ColorLogClient::GetInstance()->SendLogWithLock(severity(), file(), line(), func_name(), content());
        }
    }

    // keep vlog as is
    if (level >= 0 && level < GetMinLogLevel()) {
        return;
    }
...
LogMessage::LogMessage(const char* file, int line, const char* func_name, LogSeverity severity) {
    _stream = CreateLogStream(file, line, func_name, severity);
}

LogMessage::LogMessage(const char* file, int line, const char* func_name, std::string* result) {
    _stream = CreateLogStream(file, line, func_name, BLOG_FATAL);
...
Win32ErrorLogMessage::Win32ErrorLogMessage(const char* file,
                                           ...
                                           SystemErrorCode err)
    : err_(err),
      log_message_(file, line, func_name, severity) {
}
