...
int Server::AddBuiltinServices() {
    ...
    auto registry = prometheus::FamilyContainer::GetInstance()->GetRegistry();
    CHECK(registry) << "register new failed";
    PrometheusMetricsService* service = new (std::nothrow) PrometheusMetricsService(this);
    service->RegisterCollectable(registry);

    if (AddBuiltinService(service)) {
        LOG(ERROR) << "Fail to add MetricsService";
        return -1;
    } else {
        LOG(INFO) << "Succ to add MetricsService";
    }
